...
referencian = np.ones(time.shape)              # referencia do ponto de equilíbrio
referenciatp = (1+ 0.02)*np.ones(time.shape)   # referencia da margem de +2%
referenciatn = (1- 0.02)*np.ones(time.shape)   # referencia da margem de -2%

plt.figure(8)
# Controlador 1 (P_zn)
plt.subplot(2,2,1)
plt.plot(time,yn_P_ZN,label='c1')
plt.plot(time, referencian*yn_P_ZN[24999], "--r", label = 'referencia')
plt.plot(time, referenciatp*yn_P_ZN[24999], "--k", label = '$\pm 2\%$ referencia')
plt.plot(time, referenciatn*yn_P_ZN[24999], "--k")
plt.ylabel('y(t)[rad]')
plt.xlim(15.0,19)
plt.ylim(0.0,1.40)
plt.legend()
...
plt.plot(time, referencian*yn_PI_ZN[24999], "--r", label = 'referencia')
plt.plot(time, referenciatp*yn_PI_ZN[24999], "--k", label = '$\pm 2\%$ referencia')
plt.plot(time, referenciatn*yn_PI_ZN[24999], "--k")
plt.xlim(14.7,22)
plt.ylim(0.0,1.75)
plt.legend()
plt.grid()
...
plt.plot(time, referencian*yn_P_CHR[24999], "--r", label = 'referencia')
plt.plot(time, referenciatp*yn_P_CHR[24999], "--k", label = '$\pm 2\%$ referencia')
plt.plot(time, referenciatn*yn_P_CHR[24999], "--k")
plt.ylabel('y(t)[rad]')
plt.xlabel('Tempo [s]')
plt.xlim(15,17)
plt.ylim(0.0,0.9)
...
plt.plot(time, referencian*yn_PI_CHR[24999], "--r", label = 'referencia')
plt.plot(time, referenciatp*yn_PI_CHR[24999], "--k", label = '$\pm 2\%$ referencia')
plt.plot(time, referenciatn*yn_PI_CHR[24999], "--k")
plt.xlabel('Tempo [s]')
plt.xlim(15,18.5)
plt.ylim(0.0,1.35)
plt.legend()
...
# Para todos os métdos abaixo, os 4 primeiros valores dos vetores estão relacionados ao seguimento de referencia e os 4 últimos estão relacionados à rejeição de perturbação.
# Calculando o erro em relação à referencia 
erro_0 = np.abs(y_P_ZN[0] - referencia_3)      # Calculo do erro do Controlador Proporcional - Zigler-Nichols - seguimento de referencia
erro_1 = np.abs(y_PI_ZN[0] - referencia_3)     # Calculo do erro do Controlador Proporcional Integral - Zigler-Nichols - seguimento de referencia
erro_2 = np.abs(y_P_CHR[0] - referencia_3)     # Calculo do erro do Controlador Proporcional - CHR - seguimento de referencia
erro_3 = np.abs(y_PI_CHR[0] - referencia_3)    # Calculo do erro do Controlador Proporcional Integral - seguimento de referencias
erro_4 = np.abs(y1_[0] - referencia_4)         # Calculo do erro do Controlador Proporcional - Zigler-Nichols - rejeição à perturbação
erro_5 = np.abs(y2_[0] - referencia_4)         # Calculo do erro do Controlador Proporcional Integral - Zigler-Nichols - rejeição à perturbação
erro_6 = np.abs(y3_[0] - referencia_4)         # Calculo do erro do Controlador Proporcional - CHR - rejeição à perturbação
erro_7 = np.abs(y4_[0] - referencia_4)         # Calculo do erro do Controlador Proporcional Integral - CHR - rejeição à perturbação

# Cálculo pelo método IAE 
IAE = np.zeros(8) # Alocando um vetor de 8 posições para o erro pelo método de IAE

IAE[0] = T*np.sum(erro_0)               # Calculo do IAE do Controlador Proporcional - Zigler-Nichols - seguimento de referencia
IAE[1] = T*np.sum(erro_1)               # Calculo do IAE do Controlador Proporcional Integral - Zigler-Nichols - seguimento de referencia
IAE[2] = T*np.sum(erro_2)               # Calculo do IAE do Controlador Proporcional - CHR - seguimento de referencia
IAE[3] = T*np.sum(erro_3)               # Calculo do IAE do Controlador Proporcional Integral - seguimento de referencias
...
print(f"Seguimento de referencia - IAE - ZN  - Proporcional:             {IAE[0]}")
print(f"Seguimento de referencia - IAE - ZN  - Proporcional Intregratal: {IAE[1]}")
print(f"Seguimento de referencia - IAE - CHR - Proporcional:             {IAE[2]}")
print(f"Seguimento de referencia - IAE - CHR - Proporcional Integral:    {IAE[3]}")
...
RMSE[0] = np.sqrt((erro_0**2).mean()) # Calculo do RMSE do Controlador Proporcional - Zigler-Nichols - seguimento de referencia
RMSE[1] = np.sqrt((erro_1**2).mean()) # Calculo do RMSE do Controlador Proporcional Integral - Zigler-Nichols - seguimento de referencia
RMSE[2] = np.sqrt((erro_2**2).mean()) # Calculo do RMSE do Controlador Proporcional - CHR - seguimento de referencia
RMSE[3] = np.sqrt((erro_3**2).mean()) # Calculo do RMSE do Controlador Proporcional Integral - CHR - seguimento de referencia
